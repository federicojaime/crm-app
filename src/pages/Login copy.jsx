...
const LoginForm = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      // Guardar preferencia de "recordar" si está marcada
      ...
      setError(typeof err === 'string' ? err : 'Error al iniciar sesión, intente nuevamente');
    } finally {
      setIsLoading(false);
    }
  };
        ...
        >
          <form onSubmit={handleSubmit} className="space-y-6">
            ...
            <motion.button
              ...
              whileTap={{ scale: 0.98 }}
              className="w-full py-3 px-4 bg-blue-500 rounded-lg text-white font-medium shadow-lg shadow-blue-500/30 transition-all hover:bg-blue-600 disabled:opacity-70"
              style={{
                background: isLoading ? 'linear-gradient(to right, #4299e1, #3182ce)' : 'linear-gradient(to right, #4299e1, #3182ce)'
              }}
              disabled={isLoading}
            >
